...
#                 print("Error for: %s" % item)
#                 print("titem: %s" % titem)
#                 print("res: %s" % res)
#                 print("status: %s" % status)
#                 err = yicespy.yices_error_string()
#                 print("err: %s" % err)
#                 titem_str = yicespy.yices_term_to_string(titem, 100, 100, 0)
#                 print("titem_str: %s" % titem_str)
#                 res = yicespy.yices_formula_true_in_model(self.yices_model, titem)
#                 print("res: %s" % res)
#                 yices_res = yicespy.yices_get_value_as_term(self.yices_model, titem)
#                 print("yices_res: %s" % yices_res)
#                 result = self.converter.back(yices_res, model=self.yices_model)
...
#                 titem_str = yicespy.yices_term_to_string(titem, 100, 100, 0)
#                 print("titem_str: %s" % titem_str)
#                 titem_type = yicespy.yices_type_of_term(titem)
#                 titem_type_str = yicespy.yices_type_to_string(titem_type, 100, 100, 0)
#                 print("titem_type_str: %s" % titem_type_str)
# 
#                 if item.is_function_application():
#                     itemt = item.function_name()
#                     print("itemt: %s" % itemt)
#                     titemt = self.get_term(itemt)
#                     titemt_str = yicespy.yices_term_to_string(titemt, 100, 100, 0)
#                     print("titemt_str: %s" % titemt_str)
# 
#                     resv = yicespy.yval_t()
#                     status = yicespy.yices_get_value(self.yices_model, titemt, resv)
#                     print("status: %s" % status)
#                     print("resv: %s" % resv)
#                     mappingv = yicespy.yval_vector_t()
#                     yicespy.yices_init_yval_vector(mappingv)
#                     defv = yicespy.yval_t()
#                     tt = yicespy.make_empty_type_array(1)
#                     tt[0] = titem_type
#                     
#                     status = yicespy.yices_val_expand_function(self.yices_model, resv, defv, mappingv)
#                     print("status: %s" % status)
# 
#                     status = yicespy.yices_val_get_scalar(self.yices_model, defv, res, tt)
#                     print("status: %s" % status)
#                     print("defv: %s" % res)
#                     
#                     for i in range(mappingv.size):
#                         m = mappingv.data[i]
#                         k = yices_val_mapping_arity(self.yices_model, m)
#                         tup = yicespy.make_empty_yval_array(k)
#                         val = yicespy.yval_t()
#                         status = yicespy.yices_val_expand_mapping(self.yices_model, m, tup, val)
#                         print("status: %s" % status)
#                         status = yicespy.yices_val_get_scalar(self.yices_model, val, res, tt)
#                         print("status: %s" % status)
#                         print("val: %s" % val)
#                         
#                     yicespy.yices_delete_yval_vector(mappingv)
#                
#                 for i in range(len(ty._domain)):
#                     rhs = self.mgr.Enum(ty._domain[i], ty)
#                     print("rhs: %s" % rhs)
#                     eq = self.mgr.EqualsOrIff(item, rhs)
#                     print("eq: %s" % eq)
#                     value = self.get_value(eq, True)
#                     print("value: %s" % value)
#                 self.print_model()
#                 assert(0)
#                 yices_res = yicespy.yices_get_value_as_term(self.yices_model, titem)
