...
class YicesModel(Model):
    ...
    def print_model(self):    
        buf = cStringIO()
        yicespy.yices_print_model(buf, self.yices_model)
        res = buf.getvalue()
        buf.close()
        print(res)
...
#         self._assert_no_function_type(item)
        ...
        if ty.is_bool_type():
            status, res = yicespy.yices_get_bool_value(self.yices_model, titem)
            self._check_error(status)
            return self.mgr.Bool(bool(res))
        elif ty.is_int_type():
            res = yicespy.yices_get_integer_value(self.yices_model, titem)
            return self.mgr.Int(res)
        elif ty.is_real_type():
            status, val = yicespy.yices_get_rational_value(self.yices_model, titem)
            self._check_error(status)
            return self.mgr.Real(Fraction(val))
        elif ty.is_bv_type():
            status, res = yicespy.yices_get_bv_value(self.yices_model, titem,  ty.width)
            self._check_error(status)
            str_val = "".join(str(x) for x in reversed(res))
            return self.mgr.BV("#b" + str_val)
        elif ty.is_enum_type():
            status, val = yicespy.yices_get_scalar_value(self.yices_model, titem)
            self._check_error(status)
            assert(val >= 0)
            assert(val < len(ty._domain))
            return self.mgr.Enum(ty._domain[val], ty)
        else:
    ...
    def __iter__(self):
        ...
        """
        tv = yicespy.term_vector_t()
        yicespy.yices_model_collect_defined_terms(self.yices_model, tv)
        for d in tv:
            try:
